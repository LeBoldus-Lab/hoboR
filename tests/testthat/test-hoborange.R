...
test_that("hoborange handles dates out of range", {
  expect_error(hoborange(df, start = "2010-01-01 01:00", end = "2022-01-01 01:00"), "out of range")
}) 

test_that("hoborange errors on reversed date range", {
  expect_error(hoborange(df, start = "2010-03-01 01:00", end   = "1920-01-01 00:00"), "out of range")
})

test_that("hoborange errors on invalid date format", {
  expect_error(hoborange(df, start = "1910-01-01 00:00", end   = "invalid-date"))
})

test_that("hoborange handles empty data frame", {
  empty_df <- data.frame(Date = as.POSIXct(character()), Temperature = numeric())
  expect_error(hoborange(empty_df, start = "1910-01-01 00:00", end = "1920-01-01 00:00"), "out of range")
})
...
test_that("hoborange handles NA values correctly", {
  ...
  df_with_na <- data.frame(
    Date = as.POSIXct(
      c("1910-01-01 00:00", "1915-01-01 00:00", "1920-01-01 00:00", "1925-01-01 00:00"), tz = "UTC"),
    Temperature = c(15.5, NA, 14.8, 15.0))
  
  start <- min(df_with_na$Date, na.rm = TRUE)
  end   <- max(df_with_na$Date, na.rm = TRUE)
  
  result_na_rm_true  <- hoborange(df_with_na, start = start, end = end, na.rm = TRUE)
  result_na_rm_false <- hoborange(df_with_na, start = start, end = end, na.rm = FALSE)
