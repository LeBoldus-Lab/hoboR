...
set.seed(123)
times <- seq(from = as.POSIXct("2023-06-01 01:00:00", tz = "UTC"), 
             by = "hour", length.out = 10)
list.data <- list(
  data.frame(Date = times, Temp = rnorm(10, 20, 1), Rain = rnorm(10, 50, 5), 
             RH = rnorm(10, 90, 10)),
  data.frame(Date = times, Temp = rnorm(10, 21, 1), Rain = rnorm(10, 55, 5), 
             RH = rnorm(10, 88, 10)),
  data.frame(Date = times, Temp = rnorm(10, 19, 1), Rain = rnorm(10, 52, 5), 
             RH = rnorm(10, 91, 10))
)
...
test_that("correction.test calculates differences and checks thresholds correctly", {
  expect_message(result <- correction.test(list.data, calibrationfile, w.var = w.var, times = times_of_interest, threshold = threshold))
...
test_that("correction.test handles threshold values correctly", {
  expect_message(result <- correction.test(list.data, calibrationfile, w.var = w.var, times = times_of_interest, threshold = c(10, 10, 10)))
  
  expect_equal(all(result[[1]][1] == "passed"), TRUE)
  expect_equal(all(result[[1]][2] == "passed"), TRUE)
  expect_equal(all(result[[1]][3] == "passed"), TRUE)
  expect_equal(all(result[[1]][4] == "passed"), TRUE)
  expect_equal(all(result[[1]][8] == "passed"), FALSE)
  expect_equal(all(result[[1]][9] == "passed"), FALSE)
})
